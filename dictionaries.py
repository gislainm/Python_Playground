...
for dict in (freelancers, antiques, pet_shop):
    dict_name = dict.pop('name')
    buy_item = input(
        f'Welcome to {dict_name}! what do you want to buy: {dict.items()}: ').lower()
    if buy_item in dict:
        cart.update({buy_item: dict.pop(buy_item)})
    else:
        continue

payment = sum(list(cart.values()))
item_bought = ", ".join(list(cart.keys()))
print(f'You Purchased {item_bought}. Today they price is {payment}, you have {purse-payment} gold left. Have a nice day of mayhem!')
